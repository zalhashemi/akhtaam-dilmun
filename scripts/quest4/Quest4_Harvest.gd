...
# Result UI (make sure these nodes exist)
...
func show_feedback(msg: String) -> void:
	...
	# Restart timer so messages don't overlap
...
func update_ui() -> void:
	...
	score_label.text = "Harvest: %d / %d" % [total_now, total_needed]
...
func spawn_item(item_type: String) -> void:
	...
	# Random X between spawn markers
	...
	# Connect signals
	# dropped fires on mouse release AND when it falls into a basket (from the updated HarvestItem.gd)
	item.dropped.connect(_on_item_dropped)

	# missed fires when it falls off-screen (from the updated HarvestItem.gd)
	if item.has_signal("missed"):
		item.missed.connect(_on_item_missed)

	# fell_into_basket fires for all items; we only punish rotten in that handler
...
func _on_item_dropped(item: Node, basket: Node) -> void:
	...
	# Dropped in mid-air (basket null) -> do nothing, it keeps falling
	if basket == null:
		return

	# Rotten into any basket -> lose life
	if t == "rotten":
		lives -= 1
		show_feedback("Spoiled fruit! -1 life")
		update_ui()
		check_game_over()
		return

	# Basket must be valid
	if not (basket is Area2D):
		return

	# Read basket's accepts_type
	...
	if accepts_type == t:
		...
		lives -= 1
		show_feedback("Wrong basket! -1 life")
...
func _on_item_missed(item: Node) -> void:
	...
	lives -= 1
	show_feedback("You missed it! -1 life")
	update_ui()
	check_game_over()


# Fires when an item FALLS into basket without user releasing it.
# Only punish rotten here.
func _on_item_fell_into_basket(item, basket) -> void:
	...
	if item.item_type == "rotten":
		lives -= 1
		show_feedback("Spoiled fruit fell in! -1 life")
		update_ui()
		check_game_over()
