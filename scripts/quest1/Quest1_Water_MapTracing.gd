...
var used_end_nodes: Dictionary = {}   # instance_id -> true
...
func _ready() -> void:
	...
	facts = [
		"Water was vital to Dilmun for farming and trade.",
		"Dilmun thrived because of its freshwater springs.",
		"Modern Bahrain stands where Dilmun once flourished.",
		"Canals helped connect inland springs to coastal settlements.",
		"Freshwater supported date palms and early agriculture.",
		"Water routes strengthened trade by keeping communities supplied."
	]
	facts.shuffle()
	fact_index = 0

	message_label.text = "Draw a route from the Port (buildings with the anchor)\n to one of the landmarks!\n Make sure you stay between the lines.\n You have 3 tries"
	_clear_line()
...
func _end_draw() -> void:
	...
	if start_kind == "start" and ended_on_end != null:
		...
		connected = true
		# still consider the "port" as the EndDot you started on or ended on?
		# We'll treat "success" as connecting to a port, so determine the port node:
		# If they started on a port (start_kind == "end"), we need to lock THAT port.
		# So we lock the nearest EndDot to the START of the stroke:
	...
	# If we’re locking a port, it must be unique
	if end_node != null:
		var end_id := end_node.get_instance_id()
		if used_end_nodes.has(end_id):
			# Half-line style: vanish, no penalty (you can also message them)
			_clear_line()
			return
	...
	for lp in player_line.points:
		...
		if not _is_point_allowed(gp):
			_fail_attempt("Tracing outside breaks the flow!")
			return
...
func _on_connection_success() -> void:
	# Clear only current drawing points; frozen lines remain
	_clear_line()
	...
	if used_end_nodes.size() >= 6:
		message_label.text = "All ports are connected. Water routes restored across Dilmun!"
...
func _fail_attempt(reason: String) -> void:
	...
	if tries_left <= 0:
		...
		message_label.text = reason + "\nTries left: %d/%d" % [tries_left, max_tries]
...
func _update_tries() -> void:
	tries_label.text = "Tries: %d/%d" % [tries_left, max_tries]
